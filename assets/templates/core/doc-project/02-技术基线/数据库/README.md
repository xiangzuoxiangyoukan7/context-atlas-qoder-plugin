...
Database、Oracle PDB、Schema 或 Namespace 统一作为数据源元数据中的可选字段，不创建独立数据库单元或命名空间知识项。

功能、接口和任务使用 `rel_reads`、`rel_writes` 或 `rel_depends_on` 引用数据表；表不反向维护业务消费者。表只通过 `rel_belongs_to` 直接指向所在 `DS-*/README.md`，不再通过 `rel_classified_under` 直接连接数据库根索引；数据源 README 再分类到本索引。子表使用 `rel_logical_parent` 引用主表，并在正文精确链接主字段块锚点。
