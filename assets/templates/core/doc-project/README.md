...
| `03-变更与证据` | 当前变更、规格 Delta、验收契约、验收矩阵、实际证据和待确认知识 | 不替代长期产品定义或外部任务系统 |
...
模块、接口和独立契约一项一文件。数据库默认按数据源建立目录并一张表一个文件；只有真实拓扑需要时才增加数据库单元或命名空间层级。正式知识可以保存需求、设计、实现约束、数据结构、决策和验收证据，但不能保存秘密、未脱敏个人数据、未验证猜测、无长期价值的日志或外部任务调度状态。
