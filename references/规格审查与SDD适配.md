...
需求规格完整度与功能承接、实现状态相互独立。需求正文的问题价值、范围、稳定业务规则、成功标准、约束依赖和阻塞问题已经完整时可以为 `ready`；不存在功能的 `rel_satisfies` 只形成 `uncovered` 覆盖信息，不得作为结构错误、不得把需求降级，也不得解释为已经开发。格式 12 的需求业务内容以 Markdown 正文为唯一权威，Front Matter 只承担机器身份、状态、分类和更新时间。
